...
record finCellHom (m : ℕ) {C : CWskel ℓ} {D : CWskel ℓ'} (f g : finCellMap m C D) : Type (ℓ-max ℓ ℓ') where
  open FinSequenceMap
  field
    fhom : (n : Fin (suc m)) (x : C .fst (fst n)) → CW↪ D (fst n) (f .fmap n x) ≡ CW↪ D (fst n) (g .fmap n x)
...
finCellHom↓ : {m : ℕ} {C : CWskel ℓ} {D : CWskel ℓ'} {f g : finCellMap (suc m) C D}
  → finCellHom (suc m) f g → finCellHom m (finCellMap↓ f) (finCellMap↓ g)
finCellHom.fhom (finCellHom↓ ϕ) n x = finCellHom.fhom ϕ (injectSuc n) x 
finCellHom.fcoh (finCellHom↓ {m = suc m} ϕ) n x = finCellHom.fcoh ϕ (injectSuc n) x
...
module preChainHomotopy (m : ℕ) (C : CWskel ℓ) (D : CWskel ℓ') (f g : finCellMap m C D) (H : finCellHom m f g) (n : Fin m) where
  open FinSequenceMap
  open finCellHom
  ...
  Hn+1/Hn (merid (inr x) i) =
    ((push (f .fmap (fsuc n) x)) ∙∙ (cong inr (H .fhom (fsuc n) x)) ∙∙ (sym (push (g .fmap (fsuc n) x)))) i
  ...
  bouquetHomotopy = Iso.fun bouquetIso ∘ Hn+1/Hn ∘ Iso.inv (cofibIso (fst n) C)
    where
      bouquetIso = BouquetIso-gen (suc (fst n)) (CWskel-fields.card D (suc (fst n)))
                    (CWskel-fields.α D (suc (fst n))) (CWskel-fields.e D (suc (fst n)))
...
module MMmaps (C : CWskel ℓ) (D : CWskel ℓ') (m : ℕ) (n : Fin m) where
  MMmap : (m1 m2 : (x : C .fst (suc (fst n))) → cofibCW (fst n) D) → Type (ℓ-max ℓ ℓ')
  MMmap m1 m2 = (x : C .fst (fst n)) → m1 (CW↪ C (fst n) x) ≡ m2 (CW↪ C (fst n) x)
  ...
  MMΣcellMap f x = sym (push (f .FinSequenceMap.fmap (injectSuc n) x) ∙ (cong inr (f .FinSequenceMap.fcomm n x)))

  -- Addition of MMmaps
  MMmap-add : (m1 m2 m3 : (x : C .fst (suc (fst n))) → cofibCW (fst n) D)
...
module MMchainHomotopy* (m : ℕ) (C : CWskel ℓ) (D : CWskel ℓ') (f g : finCellMap m C D) (H : finCellHom m f g) (n : Fin m) where
  open FinSequenceMap
  open finCellHom
  open MMmaps C D
  ...
  MM∂H x = (sym (cong inr (f .fcomm n x))) ∙∙ (cong inr (fhom H (injectSuc n) x)) ∙∙ (cong inr (g .fcomm n x))
  ...
  MMΣH∂ x = sym ((push (f .fmap (injectSuc n) x)) ∙∙ (cong inr (H .fhom (injectSuc n) x)) ∙∙ (sym (push (g .fmap (injectSuc n) x))))

  -- the chain homotopy equation at the level of MMmaps
  MMchainHomotopy : ∀ x →
    MMmap-add m n merid-f merid-tt merid-tt (MMmap-add m n merid-f merid-g merid-tt MM∂H MMΣg) MMΣH∂ x
    ≡ MMΣf x
  MMchainHomotopy x = sym (doubleCompPath-elim (MM∂H x) (MMΣg x) (MMΣH∂ x)) ∙ aux2
    where
      aux : Square (MMΣf x) (MMΣg x) (MM∂H x) (sym (MMΣH∂ x))
      aux i j =
         hcomp (λ k → λ {(i = i0) → compPath-filler (push (f .fmap (injectSuc n) x))
...
module realiseMMmap (C : CWskel ℓ) (D : CWskel ℓ') (m : ℕ) (f g : finCellMap m C D) (H : finCellHom m f g) (n : Fin m)  where
  open FinSequenceMap
  open finCellHom
  open MMmaps C D
  ...
  realiseMMmap2 : (n : Fin m) → (m1 m2 : (x : C .fst (suc (fst n))) → cofibCW (fst n) D)
  ...
  realiseMMmap1≡2 : (n : Fin m) → (m1 m2 : (x : C .fst (suc (fst n))) → cofibCW (fst n) D) (e : MMmap m n m1 m2)
    (x : Susp (cofibCW (fst n) C)) → realiseMMmap m n m1 m2 e x ≡ realiseMMmap2 n m1 m2 e x
  ...
  realiseMMΣcellMap : (f : finCellMap m C D) (x : Susp (cofibCW (fst n) C)) →
        realiseMMmap m n (λ x → (inr (f .fmap (fsuc n) x))) (λ x → inl tt) (MMΣcellMap m n f) x
        ≡ suspFun (prefunctoriality.fn+1/fn m f n) x
  realiseMMΣcellMap f x = realiseMMmap1≡2 n (λ x → (inr (f .fmap (fsuc n) x))) (λ x → inl tt) (MMΣcellMap m n f) x ∙ aux x
    where
      aux : (x : Susp (cofibCW (fst n) C)) →
        realiseMMmap2 n (λ x → (inr (f .fmap (fsuc n) x))) (λ x → inl tt) (MMΣcellMap m n f) x
        ≡ suspFun (prefunctoriality.fn+1/fn m f n) x
      aux north = refl
      aux south l = merid (inl tt) l
      aux (merid (inl tt) i) l = merid (inl tt) (i ∧ l)
      aux (merid (inr x) i) l =
        hcomp (λ k → λ { (i = i0) → merid (inr (f .fmap (fsuc n) x)) (~ k)
                       ; (i = i1) → merid (inl tt) (l ∨ (~ k))
                       ; (l = i1) → merid (inr (f .fmap (fsuc n) x)) (~ k ∨ i) })
              south
      aux (merid (push x j) i) l =
        hcomp (λ k → λ { (i = i0) → merid ((push (f .fmap (injectSuc n) x) ∙ (cong inr (f .fcomm n x))) j) (~ k)
                       ; (i = i1) → merid (inl tt) (l ∨ (~ k))
                       ; (j = i0) → merid (inl tt) ((i ∧ l) ∨ (~ k))
                       ; (l = i1) → merid ((push (f .fmap (injectSuc n) x) ∙ (cong inr (f .fcomm n x))) j) (i ∨ (~ k)) })
              south
  ...
  cof∂H (merid (inr x) i) = ((merid (inr (f .fmap (fsuc n) x))) ∙∙ refl ∙∙ (sym (merid (inr (g .fmap (fsuc n) x))))) i
  ...
  realiseMM∂H x = aux2 x ∙ aux x
    where
      aux : (x : Susp (cofibCW (fst n) C)) → cof∂H x ≡ suspFun (to_cofibCW (fst n) D) (δ (suc (fst n)) D (Hn+1/Hn n x))
      aux north = refl
      aux south = refl
      ...
      aux (merid (push x j) i) k =
        hcomp (λ l → λ { (i = i0) → merid (inr (f .fcomm n x j)) (~ l)
      ...
      aux2 : (x : Susp (cofibCW (fst n) C)) →
        realiseMMmap m n (λ x → (inr (f .fmap (fsuc n) x))) (λ x → (inr (g .fmap (fsuc n) x))) (MM∂H n) x
        ≡ cof∂H x
      aux2 north = refl
      aux2 south = refl
      ...
      aux2 (merid (push x j) i) l =
        hcomp (λ k → λ { (i = i0) → merid (inr (f .fcomm n x (j ∨ (~ l)))) (~ k)
                       ; (i = i1) → merid (inr (g .fcomm n x (j ∨ (~ l)))) (~ k)
                       ; (j = i0) → merid (doubleCompPath-filler (sym (cong inr (f .fcomm n x)))
...
module realiseMMmapP2 (C : CWskel ℓ) (D : CWskel ℓ') (m : ℕ) (f g : finCellMap (suc m) C D) (H : finCellHom (suc m) f g) (n : Fin m)  where
  open FinSequenceMap
  open finCellHom
  open MMmaps C D
  ...
  Hn+1/Hn' : {!preChainHomotopy.Hn+1/Hn m C D (finCellMap↓ f) (finCellMap↓ g) (finCellHom↓ H) n!}
  Hn+1/Hn' = preChainHomotopy.Hn+1/Hn m C D (finCellMap↓ f) (finCellMap↓ g) (finCellHom↓ H) n

  realiseMMΣH∂* : {!!}
  realiseMMΣH∂* = {!!}

  realiseMMΣH∂ : (x : Susp (cofibCW (suc (fst n)) C)) →
        realiseMMmap (suc m) (fsuc n) (λ x → inl tt) (λ x → inl tt) (MMΣH∂ (fsuc n)) x
        ≡ suspFun (Hn+1/Hn'
                 ∘ suspFun (to_cofibCW (fst n) C)
                 ∘ δ (suc (fst n)) C) x
  realiseMMΣH∂ x =
    realiseMMmap1≡2 fzero (fsuc n) (λ x → inl tt) (λ x → inl tt) (MMΣH∂ (fsuc n)) x
    ∙ aux x
    where
      aux : (x : Susp (cofibCW (suc (fst n)) C)) →
        realiseMMmap.realiseMMmap2 C D (suc m) f g H fzero (fsuc n)
          (λ x₁ → inl tt) (λ x₁ → inl tt)
          (MMchainHomotopy*.MMΣH∂ (suc m) C D f g H (fsuc n)) x
        ≡ suspFun (Hn+1/Hn' ∘ (suspFun (to_cofibCW (fst n) C)) ∘ (δ (suc (fst n)) C)) x
      aux north = refl
      aux south l = merid (inl tt) l
      aux (merid (inl tt) i) l = merid (inl tt) (i ∧ l)
      aux (merid (inr x) i) l =
        hcomp (λ k → λ { (i = i0) → merid (inl tt) (~ k)
              ...
              south
      aux (merid (push x j) i) l =
        hcomp (λ k → λ { (i = i0) → merid (((push (f .fmap (injectSuc (fsuc n)) x))
                         ...
                         ; (l = i1) → merid (((push (f .fmap (injectSuc (fsuc n)) x))
                                    ∙∙ (cong inr (H .fhom (injectSuc (fsuc n)) x))
                                    ∙∙ (sym (push (g .fmap (injectSuc (fsuc n)) x)))) j) (i ∨ (~ k))})
                south
...
module bouquetAdd where
  ...
  module _ (C : CWskel ℓ) (D : CWskel ℓ') (m : ℕ) (n : Fin m) (m1 m2 : (x : C .fst (suc (fst n))) → cofibCW (fst n) D)
            (f : MMmap C D m n m1 m2)
            (a : CWskel-fields.A D (fst n)) where

    bouquetMMmap∈cohom-raw : (t : CWskel-fields.A C (fst n)) → S₊ (suc (fst n)) → S₊ (suc (fst n))
    bouquetMMmap∈cohom-raw t x = pickPetal a (bouquetMMmap C D m n m1 m2 f (inr (t , x)))

    bouquetMMmap∈cohom : (t : CWskel-fields.A C (fst n))  → S₊ (suc (fst n)) → coHomK (suc (fst n))
    bouquetMMmap∈cohom t x = ∣ bouquetMMmap∈cohom-raw t x ∣ₕ

    bouquetMMmap∈cohom' : (x : Susp (cofibCW (fst n) C)) → coHomK (suc (fst n))
    bouquetMMmap∈cohom' x = ∣ pickPetal a (Iso.fun (cofibIso (fst n) D) (realiseMMmap C D m n m1 m2 f x)) ∣ₕ

  --
  realiseAdd-merid : (C : CWskel ℓ) (D : CWskel ℓ') (m : ℕ) (n : Fin m) (m1 m2 m3 : (x : C .fst (suc (fst n))) → cofibCW (fst n) D)
            (f : MMmap C D m n m1 m2)
  ...
  realiseAdd-merid C D m n m1 m2 m3 f g (push a l) i j =
    hcomp (λ k → λ { (i ∨ j = i0) → merid (m1 (CW↪ C (fst n) a)) (~ k)
  ...
  bouquetMMmap∈cohom'+ : (C : CWskel ℓ) (D : CWskel ℓ') (m : ℕ) (n : Fin m) (m1 m2 m3 : (x : C .fst (suc (fst n))) → cofibCW (fst n) D)
            (f : MMmap C D m n m1 m2)
            (g : MMmap C D m n m2 m3)
  ...
  bouquetMMmap∈cohom'+ C D m (zero , p) m1 m2 m3 f g a (merid b i) j =
    ...
    where
    help : (b : _)
      → PathP (λ i → ∣ base ∣ₕ ≡ cong (bouquetMMmap∈cohom' C D m (zero , p) m2 m3 g a) (merid b) i)
           (cong (bouquetMMmap∈cohom' C D m (zero , p) m1 m2 f a) (merid b))
           (cong (bouquetMMmap∈cohom' C D m (zero , p) m1 m3 (MMmap-add C D m (zero , p) m1 m2 m3 f g) a) (merid b))
    help b i j = ∣ pickPetal a (Iso.fun (cofibIso zero D) (realiseAdd-merid C D m (zero , p) m1 m2 m3 f g b i j)) ∣ₕ
  ...
  bouquetMMmap∈cohom'+ C D m (suc n , p) m1 m2 m3 f g a (merid b i) j =
    ...
    where
    help : (b : _)
      → PathP (λ i → ∣ north ∣ₕ ≡ cong (bouquetMMmap∈cohom' C D m (suc n , p) m2 m3 g a) (merid b) i)
           (cong (bouquetMMmap∈cohom' C D m (suc n , p) m1 m2 f a) (merid b))
           (cong (bouquetMMmap∈cohom' C D m (suc n , p) m1 m3 (MMmap-add C D m (suc n , p) m1 m2 m3 f g) a) (merid b))
    help b i j = ∣ pickPetal a (Iso.fun (cofibIso (suc n) D) (realiseAdd-merid C D m (suc n , p) m1 m2 m3 f g b i j)) ∣ₕ
  ...
  bouquetMMmap∈cohom+ : (C : CWskel ℓ) (D : CWskel ℓ') (m : ℕ) (n : Fin m) (m1 m2 m3 : (x : C .fst (suc (fst n))) → cofibCW (fst n) D)
            ...
            (a : CWskel-fields.A D (fst n))
            (x : S₊ (suc (fst n)))
         → bouquetMMmap∈cohom C D m n m1 m3 (MMmap-add C D m n m1 m2 m3 f g) a t x
  ...
  bouquetMMmap∈cohom+ C D m n m1 m2 m3 f g t a x =
    bouquetMMmap∈cohom'+ C D m n m1 m2 m3 f g a (Iso.inv (cofibIso (fst n) C) (inr (t , x)))

  module _  (C : CWskel ℓ) (D : CWskel ℓ') (m : ℕ) (n : Fin m) (m1 m2 m3 : (x : C .fst (suc (fst n))) → cofibCW (fst n) D)
            (f : MMmap C D m n m1 m2) (g : MMmap C D m n m2 m3) where
    realiseMMmap-hom : bouquetDegree (bouquetMMmap C D m n m1 m3 (MMmap-add C D m n m1 m2 m3 f g))
      ...
      agreeOnℤFinGenerator→≡ λ t → funExt λ a
        → sym (isGeneratorℤFinGenerator'
                (λ a₁ → degree (suc (fst n))
                  λ x → pickPetal a (bouquetMMmap C D m n m1 m3 (MMmap-add C D m n m1 m2 m3 f g)
                           (inr (a₁ , x)))) t)
         ∙ cong (fst (Hⁿ-Sⁿ≅ℤ (fst n)) .Iso.fun ∘ ∣_∣₂)
                (funExt (bouquetMMmap∈cohom+ C D m n m1 m2 m3 f g t a))
        ∙∙ IsGroupHom.pres· (snd (Hⁿ-Sⁿ≅ℤ (fst n)))
             (∣ (λ x → ∣ pickPetal a (bouquetMMmap C D m n m1 m2 f (inr (t , x))) ∣ₕ) ∣₂)
             (∣ (λ x → ∣ pickPetal a (bouquetMMmap C D m n m2 m3 g (inr (t , x))) ∣ₕ) ∣₂)
...
module chainHomEquation (m : ℕ) (C : CWskel ℓ) (D : CWskel ℓ') (f g : finCellMap (suc m) C D) (H : finCellHom (suc m) f g) (n : Fin m) where
  open SequenceMap
  ...
  bouquet∂H =
    cong (λ X → bouquetDegree ((Iso.fun (cofibIso (suc (fst n)) D)) ∘ X ∘ (Iso.inv (cofibIso (suc (fst n)) C))))
         (funExt (realiseMMmap.realiseMM∂H C D (suc m) f g H (fsuc n)))
      ∙ cong bouquetDegree ιδH≡pre∂∘H
      ∙ bouquetDegreeComp (preboundary.pre∂ D (suc (fst n))) (bouquetHomotopy (fsuc n))
    where
      ιδH : SphereBouquet (suc (suc (fst n))) (Fin (CWskel-fields.card C (suc (fst n))))
          → SphereBouquet (suc (suc (fst n))) (Fin (CWskel-fields.card D (suc (fst n))))
      ιδH = Iso.fun (cofibIso (suc (fst n)) D) ∘ suspFun (to_cofibCW (suc (fst n)) D) ∘ δ (suc (suc (fst n))) D
            ∘ Hn+1/Hn (fsuc n) ∘ Iso.inv (cofibIso (suc (fst n)) C)

      ιδH≡pre∂∘H : ιδH ≡ (preboundary.pre∂ D (suc (fst n))) ∘ bouquetHomotopy (fsuc n)
      ιδH≡pre∂∘H = cong (λ X → Iso.fun (cofibIso (suc (fst n)) D) ∘ suspFun (to_cofibCW (suc (fst n)) D)
  ...
  bouquetΣH∂ =
    {- cong (λ X → bouquetDegree ((Iso.fun (cofibIso (suc (fst n)) D)) ∘ X ∘ (Iso.inv (cofibIso (suc (fst n)) C))))
         (funExt (realiseMMmapP2.realiseMMΣH∂ C D m f g H n)) -}
       {!bouquetMMmap merid-tt merid-tt MMΣH∂!}
      ∙ cong bouquetDegree (cofibIso-suspFun _ C D (Hn+1/Hn (injectSuc n) ∘ suspFun (to_cofibCW (fst n) C) ∘ δ (suc (fst n)) C))
      ∙ sym (bouquetDegreeSusp Hιδ)
      ∙ cong bouquetDegree Hιδ≡H∘pre∂
      ∙ bouquetDegreeComp (bouquetHomotopy (injectSuc n)) (preboundary.pre∂ C (fst n))
      {-
      ∙ cong bouquetDegree
             (cofibIso-suspFun _ C D (Hn+1/Hn (injectSuc n) ∘ suspFun (to_cofibCW (fst n) C) ∘ δ (suc (fst n)) C))
      ∙ sym (bouquetDegreeSusp Hιδ)
      ∙ cong bouquetDegree Hιδ≡H∘pre∂
      ∙ bouquetDegreeComp (bouquetHomotopy (injectSuc n)) (preboundary.pre∂ C (fst n))
      -}
    where
      ...
      Hιδ = Iso.fun (BouquetIso D (suc (fst n))) ∘ (Hn+1/Hn (injectSuc n)) ∘ suspFun (to_cofibCW (fst n) C)
            ∘ δ (suc (fst n)) C ∘ Iso.inv (BouquetIso C (suc (fst n)))

      Hιδ≡H∘pre∂ : Hιδ ≡ bouquetHomotopy (injectSuc n) ∘ (preboundary.pre∂ C (fst n))
      Hιδ≡H∘pre∂ = cong (λ X → Iso.fun (BouquetIso D (suc (fst n))) ∘ (Hn+1/Hn (injectSuc n)) ∘ X
  ...
  bouquetΣf = cong (λ X → bouquetDegree ((Iso.fun (cofibIso (suc (fst n)) D)) ∘ X ∘ (Iso.inv (cofibIso (suc (fst n)) C))))
         (funExt (realiseMMmap.realiseMMΣf C D (suc m) f g H (fsuc n)))
    ∙ (cong bouquetDegree (cofibIso-suspFun (suc (fst n)) C D (prefunctoriality.fn+1/fn (suc m) f (fsuc n))))
  ...
  bouquetΣg = cong (λ X → bouquetDegree ((Iso.fun (cofibIso (suc (fst n)) D)) ∘ X ∘ (Iso.inv (cofibIso (suc (fst n)) C))))
         (funExt (realiseMMmap.realiseMMΣg C D (suc m) f g H (fsuc n)))
    ∙ (cong bouquetDegree (cofibIso-suspFun (suc (fst n)) C D (prefunctoriality.fn+1/fn (suc m) g (fsuc n))))
  ...
  chainHomotopy1 =
    ...
    where
      ...
      aux = cong₂ (λ X Y → addGroupHom _ _ X Y) (sym bouquet∂H) (sym bouquetΣg)
            ∙ sym (bouquetAdd.realiseMMmap-hom C D (suc m) (fsuc n) T.merid-f T.merid-g T.merid-tt T.MM∂H T.MMΣg)
      ...
      aux2 = cong (addGroupHom _ _ (bouquetDegree (bouquetMMmap merid-f merid-tt MM∂H+MMΣg)))
                  (sym bouquetΣH∂)
             ∙ sym (bouquetAdd.realiseMMmap-hom C D (suc m) (fsuc n) T.merid-f T.merid-tt T.merid-tt MM∂H+MMΣg T.MMΣH∂)
  ...
  chainHomotopy2 = GroupHom≡ (funExt λ x → aux (fn+1 .fst x) (∂H .fst x) (gn+1 .fst x)
     ...
     where
      open AbGroupStr (snd (ℤ[AD (suc (fst n)) ])) renaming (_+_ to _+G_ ; -_ to -G_ ; +Assoc to +AssocG ; +Comm to +CommG)
...
cellHom-to-ChainHomotopy : {C : CWskel ℓ} {D : CWskel ℓ'} (m : ℕ) {f g : finCellMap (suc m) C D} (H : finCellHom (suc m) f g)
                         → finChainHomotopy m (finCellMap→finChainComplexMap m f) (finCellMap→finChainComplexMap m g)
cellHom-to-ChainHomotopy {C = C} {D} m {f} {g} H .finChainHomotopy.fhtpy n = preChainHomotopy.chainHomotopy (suc m) C D f g H n
cellHom-to-ChainHomotopy {C = C} {D} m {f} {g} H .finChainHomotopy.fbdryhtpy n = chainHomEquation.chainHomotopy2 m C D f g H n
