...
snd (fst (connected→π'Equiv {ℓ = ℓ} {A = A} {B = B} n f conf)) =
  subst isEquiv (π'∘∙Hom'≡π'∘∙fun n f) h1
  where
  h1 : isEquiv (π'∘∙Hom' n f .fst)
  h1 = transport (λ j → isEquiv
        (transp (λ i → GroupHomπ≅π'PathP A B n (i ∧ j)) (~ j)
            (πHom n f) .fst))
...
connected→πSurj {ℓ = ℓ}  {A = A} {B = B} n f conf = {!!}
  ...
  -- module M = πLES f
  -- fibf = fiber (fst f) (pt B)

  -- isContrFib : isContr (π (suc n) (fibf , pt A , snd f))
  -- isContrFib =
  --   subst isContr
  --      (refl
  --     ∙ cong (π (suc n)) ((sym (ua∙ (isContr→≃Unit* {ℓ' = ℓ-max ℓ ℓ'} (conf (pt B))) refl)))
  --     ∙ sym (isoToPath (fst (πTruncGroupIso
  --             {A = (fibf , pt A , snd f)} n)))) {!!}
