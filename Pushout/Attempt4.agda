...
module _ {ℓ ℓ' ℓ'' : Level} {B : CWskel ℓ} {C : CWskel ℓ'} {D : CWskel ℓ''}
  ...
  PushoutPushoutMap→PushoutA n (inl (inl x)) = inl (invEq (e C (suc n)) (inl x))
  PushoutPushoutMap→PushoutA n (inl (inr x)) = inr (invEq (e D (suc n)) (inl x))
  PushoutPushoutMap→PushoutA n (inl (push a i)) =
    ((λ i → inl (comm f (suc n) (CW↪ B n a) i))
    ∙∙ push (invEq (e B n) (inl a))
    ∙∙ λ i → inr (comm g (suc n) (CW↪ B n a) (~ i))) i
  PushoutPushoutMap→PushoutA n (inr (inl (inl x))) = inl (invEq (e C (suc n)) (inr x)) -- inr {!!}
  PushoutPushoutMap→PushoutA n (inr (inl (inr x))) = inr (invEq (e D (suc n)) (inr x)) -- inr {!!}
  PushoutPushoutMap→PushoutA n (inr (inr x)) = inl (∣ f ∣ (suc (suc n)) (invEq (e B (suc n)) {!!}))
  PushoutPushoutMap→PushoutA n (push (inl (inl x) , y) i) = inl (invEq (e C (suc n)) (push (x , Iso.inv (IsoSphereSusp n) y) i))
  PushoutPushoutMap→PushoutA n (push (inl (inr x) , y) i) = inr (invEq (e D (suc n)) (push (x , Iso.inv (IsoSphereSusp n) y) i))
  PushoutPushoutMap→PushoutA n (push (inr x , y) i) = {!!}
...
module _ (isEquivPushoutA→PushoutPushoutMapStrict : ∀ {ℓ ℓ' ℓ''} {B : CWskel ℓ} {C : CWskel ℓ'} {D : CWskel ℓ''}
  ...
  module _ {ℓ ℓ' ℓ'' : Level} {B' : CWskel ℓ} {C' : CWskel ℓ'} {D' : CWskel ℓ''}
    ...
    ∂LL ∂RR : (n : ℕ) → SplitBouquet (suc n) (card C (suc (suc n)))
                       (card D (suc (suc n))) (card (strictCWskel B') (suc n))
         → SplitBouquet (suc n) (card C (suc n)) (card D (suc n))
             (card (strictCWskel B') n)
    ∂LL n = Iso.fun (SphereBouquetCellIso (suc n) _)
                        ∘ preboundarypre∂Simpl cofibCWskel (suc n)
                        ∘ Iso.inv (SphereBouquetCellIso (suc n) _)
    ∂RR n = ((((λ x → inl (inr x))
                          , (λ i → inl (push tt (~ i))) ∙ push tt)
                           ∘∙ (preboundarypre∂Simpl D (suc n) , refl))
                           ∘∙ foldR∙)
                       ∨→ SphereBouquet∙Π
                            (((inr , refl) ∘∙ (bouquetSusp→ (preboundarypre∂Simpl B n) , refl)))
...
-- preboundarypre∂Simpl
    ∂L* ∂R* : (n : ℕ) → SplitBouquet (suc n) (card C (suc (suc n)))
                       (card D (suc (suc n))) (card (strictCWskel B') (suc n))
         → SphereBouquet (suc (suc n)) (Fin (card cofibCWskel (suc n)))
    ∂L* n =  preboundarypre∂Simpl cofibCWskel (suc n)
                        ∘ Iso.inv (SphereBouquetCellIso (suc n) _)
    ∂R* n x = Iso.inv (SphereBouquetCellIso (suc n) _) (∂RR n x)

    funH : (n : ℕ) (x : Fin (preboundary.An B (suc n))) (a : S₊ (suc n)) → fst cofibCWskel (suc (suc n))
    funH zero x base = inl tt
    funH zero x (loop i) =
      ((push (α B (suc zero) (x , false)) ∙ (λ i → inr (∣ f ∣ 2 ((push (x , false) i)))))
      ∙ (sym (push (α B (suc zero) (x , true)) ∙ (λ i → inr (∣ f ∣ 2 ((push (x , true) i))))))) i
    funH (suc n) x north = inl tt
    funH (suc n) x south = inr (∣ f ∣ (suc (suc (suc n))) (inr x))
    funH (suc n) x (merid a i) =
      (push (α B (suc (suc n)) (x , a)) ∙ λ i → inr (∣ f ∣ (suc (suc (suc n))) (push (x , a) i))) i

    pushoutMapₛ≡funH : (n : ℕ) (x : _) (a : _)
      → pushoutMapₛ terminalCW f (suc n) (inr x , Iso.fun (IsoSucSphereSusp n) a)
      ≡ funH n x a
    pushoutMapₛ≡funH zero x base = refl
    pushoutMapₛ≡funH zero x (loop i) j =
      cong-∙ (λ w → pushoutMapₛ terminalCW f 1 (inr x , w))
             (merid false) (sym (merid true)) j i
    pushoutMapₛ≡funH (suc n) x north = refl
    pushoutMapₛ≡funH (suc n) x south = refl
    pushoutMapₛ≡funH (suc n) x (merid a i) = refl

    fun : (n : ℕ) → SphereBouquet (suc n) (Fin (card B n))
                   → Susp (fst cofibCWskel (suc n))
    fun n (inl x) = north
    fun zero (inr (x , base)) = north
    fun zero (inr (x , loop i)) = toSusp (_ , (inl tt)) (inr (∣ f ∣ 1 (inr x))) i
    fun (suc n) (inr (x , north)) = north
    fun (suc n) (inr (x , south)) = south
    fun (suc n) (inr (x , merid a i)) = merid (funH n x a) i
    fun zero (push a i) = north
    fun (suc n) (push a i) = north

    fun' : (n : ℕ) → SphereBouquet (suc n) (Fin (card B n))
                   → Susp (cofibCW n cofibCWskel)
    fun' n (inl x) = north
    fun' zero (inr (x , base)) = north
    fun' zero (inr (x , loop i)) = toSusp (_ , inr (inl tt)) (inr (inr (∣ f ∣ 1 (inr x)))) i
    fun' (suc n) (inr (x , north)) = north
    fun' (suc n) (inr (x , south)) = south
    fun' (suc n) (inr (x , merid a i)) = merid (inr (funH n x a)) i
    fun' zero (push a i) = north
    fun' (suc n) (push a i) = north


    preb : (n : ℕ) → SphereBouquet n (Fin (preboundary.An B n))
         → (fst cofibCWskel (suc n))
    preb zero (inl x) = inl tt
    preb zero (inr (x , false)) = inr (∣ f ∣ 1 (inr x))
    preb zero (inr (x , true)) = inl tt
    preb zero (push a i) = inl tt
    preb (suc n) (inl x) = inl tt
    preb (suc n) (inr x) = funH n (fst x) (snd x)
    preb (suc zero) (push a i) = inl tt
    preb (suc (suc n)) (push a i) = inl tt


    bdryCofib : (n : ℕ) → SphereBouquet (suc n) (Fin (card B n))
                   → Susp (SphereBouquet n (Fin (card cofibCWskel n)))
    bdryCofib n = suspFun (preboundary.isoCofBouquet cofibCWskel n) ∘ fun' n

    bdryCofib' : (n : ℕ) → SphereBouquet n (Fin (card B n))
                   → (SphereBouquet n (Fin (card cofibCWskel n)))
    bdryCofib' n = (preboundary.isoCofBouquet cofibCWskel n) ∘ inr ∘ preb n

    bado : (n : ℕ) → cofibCW n B → cofibCW (suc n) cofibCWskel
    bado n (inl x) = inl x
    bado n (inr x) = inr (CW↪ cofibCWskel (suc n) (inr (∣ f ∣ (suc n) x)))
    bado n (push a i) = (push (inl tt) ∙ (λ i → inr (CW↪ cofibCWskel (suc n) (push a i)))) i
    {-
      (push (inr (inl (∣ f ∣ n a)))
      ∙ {!!}) i
-}
    wll : (n : ℕ) → cofibCW (suc n) D → cofibCW (suc n) cofibCWskel
    wll n (inl x) = inl tt
    wll n (inr x) = inr (inr x) -- (invEq (e cofibCWskel (suc n)) (inl {!inr x!})) -- (inr x)
    wll n (push a i) = (push (inr a) ∙ λ i → inr {!!}) i -- ((λ j → inr {!inr (retEq (e cofibCWskel n ) (inr a) j)!}) ∙ (λ j → inr {!!}) ∙ {!!}) ∙ (λ j → inr (inr (inl a))) ∙ refl) i
      where
      ho : (a : _) → Path (fst cofibCWskel (suc (suc n)))
                (CW↪ cofibCWskel (suc n) (inr a)) (inr (CW↪ D (suc n) a))
      ho a = retEq (e cofibCWskel (suc n)) _ ∙ {!secEq (e cofibCWskel (suc n)) a!}
 
    testie :  (n : ℕ) → (x : _)
      → {!fun' (suc n)!} -- (suspFun (to (suc n) cofibCW B) (δ (suc (suc n)) B (preboundary.isoCofBouquetInv↑ B (suc n) x)))
      ≡  {!suspFun (prefunctoriality.fn+1/fn (suc (suc n)) (cellMap→finCellMap ? f) flast ∘ preboundary.isoCofBouquetInv↑ B n) !} -- suspFun {!!} ? -- (suspFun (preb n) {!!}) -- fun' (suc n) {!x!}
    testie n x = {!suspFun (to (suc n) cofibCW B) (δ (suc (suc n)) B (preboundary.isoCofBouquetInv↑ B (suc n) x))!}

    funL : (n : ℕ) → (x : _) → bdryCofib' (suc n) x ≡ {!preboundary.isoCofBouquet cofibCWskel (suc n) --- suspFun (preboundary.isoCofBouquet B n) -- ∘ ((suspFun (to n cofibCW B) ∘ δ (suc n) B ∘ preboundary.isoCofBouquetInv↑ B n)) !} -- 
    funL n = {!preboundary.isoCofBouquet cofibCWskel n -- suspFun (preboundary.isoCofBouquet B n) ∘ (suspFun (to n cofibCW B) ∘ δ (suc n) B ∘ preboundary.isoCofBouquetInv↑ B n)!}


    SuspB→ : (n : ℕ) → Susp (cofibCW n B) → cofibCW (suc n) cofibCWskel
    SuspB→ n north = inl tt
    SuspB→ n south = inl tt
    SuspB→ n (merid (inl x) i) = {!cofibCW (suc n) cofibCWskel -- preb -- cofibCW (suc n) cofibCWskel!} -- inr (inl tt)
    SuspB→ n (merid (inr x) i) = {!!} -- (push (inr (∣ f ∣ (suc n) x)) ∙ {!!} ∙ (λ i → inr (push x (~ i))) ∙ {!refl!}) i -- inl tt
    SuspB→ n (merid (push a i) j) =
      hcomp (λ k → {!!})
            (push (inr {!!}) i)

      -- (push (inr (∣ f ∣ (suc n) a)) ∙ {!λ i → inr (inl ?)!} ∙ {!suspFun (suspFun (to n cofibCW B) ∘ δ (suc n) B ∘ preboundary.isoCofBouquetInv↑ B n) ∘ Iso.inv sphereBouquetSuspIso!}) i -- (λ i → inr {!comm f (suc (suc n)) (inl a) (~ i)!}) ∙∙ (λ i → inr (push a (~ i)))) i -- (push a ∙ λ i → inr ((sym (comm f (suc n) a) ∙ {!a -- push ? ∙ ?!}) i)) i

    B→cofibCWskel : (n : ℕ) → {!preboundary.pre∂ B n -- suspFun (SuspB→ n) ∘ suspFun (δ (suc n) B ∘ preboundary.isoCofBouquetInv↑ B n) ∘ Iso.inv sphereBouquetSuspIso!}
    B→cofibCWskel = {!preboundary.pre∂ cofibCWskel -- preboundary.pre∂ cofibCWskel -- preboundary.An+1 B ?!}

    prebEq : (n : ℕ) (x : _)
      → suspFun (preb n) (Iso.inv sphereBouquetSuspIso x) ≡ fun n x
    prebEq zero (inl x) = refl
    prebEq (suc n) (inl x) = refl
    prebEq zero (inr (x , base)) = refl
    prebEq zero (inr (x , loop i)) j =
      cong-∙ (λ w → suspFun (preb zero)
        (Bouquet→SuspBouquet _ (λ _ → S₊∙ zero) (inr (x , w))))
           (merid false) (sym (merid true)) j i
    prebEq (suc n) (inr (x , north)) = refl
    prebEq (suc n) (inr (x , south)) = refl
    prebEq (suc n) (inr (x , merid a i)) = refl
    prebEq zero (push a i) = refl
    prebEq (suc n) (push a i) = refl

    firstEq : (n : ℕ) (x : _)
      → fun n x
       ≡ (δ (suc n) cofibCWskel (preboundary.isoCofBouquetInv↑ cofibCWskel n
            (Iso.inv (SphereBouquetCellIso n _) (inr x))))
    firstEq zero (inl x) = refl
    firstEq zero (inr (x , base)) = refl 
    firstEq zero (inr (x , loop i)) j = he (~ j) i
      where
      he : cong (δ 1 cofibCWskel ∘ preboundary.isoCofBouquetInv↑ cofibCWskel zero) (λ i → inr (Iso.fun Iso-Fin⊎Fin-Fin+ (inr x) , loop i))
        ≡ toSusp (_ , (inl tt)) (inr (∣ f ∣ 1 (inr x)))
      he = cong-∙∙ (δ 1 cofibCWskel) _ _ _
         ∙ cong₃ _∙∙_∙∙_
                 (cong merid
                   (cong (pushoutMapₛ terminalCW f 0)
                         (ΣPathP (secEq (finSplit3≃ _ _ _) (inr x) , refl))))
                 (λ _ _ → south)
                 (cong (sym ∘ merid)
                   (cong (pushoutMapₛ terminalCW f 0)
                         (ΣPathP (secEq (finSplit3≃ _ _ _) (inr x) , refl))))
    ...
    firstEq zero (push a i) = refl
    firstEq (suc n) (inl x) = refl
    firstEq (suc n) (inr (x , north)) = refl
    firstEq (suc n) (inr (x , south)) = sym (merid (inl tt))
    firstEq (suc n) (inr (x , merid a i)) j = h (~ j) i
      where
      h : Square (cong (δ (suc (suc n)) cofibCWskel
               ∘ preboundary.isoCofBouquetInv↑ cofibCWskel (suc n))
               (λ i → inr (invEq
                  (finSplit3≃ (card C (suc (suc n))) (card D (suc (suc n)))
                    (pushoutMidCells terminalCW f (suc (suc n)))) (inr x)
                 , merid a i)))
           (λ i → fun (suc n) (inr (x , merid a i)))
           refl (merid (inl tt))
       
      h = (cong-∙∙ (δ (suc (suc n)) cofibCWskel) _ _ _
        ∙ cong₃ _∙∙_∙∙_ (cong merid (cong (pushoutMapₛ terminalCW f (suc n)) (ΣPathP (secEq (finSplit3≃ _ _ _) (inr x) , refl))))
                (λ _ _ → south)
            (cong (sym ∘ merid)
              (cong (pushoutMapₛ terminalCW f (suc n)) (ΣPathP (secEq (finSplit3≃ _ _ _) (inr x) , IsoSucSphereSusp∙' n))))
        ∙ doubleCompPath≡compPath _ _ _
         ∙ cong₂ _∙_ refl (sym (lUnit _))
         ∙ refl)
         ◁ (symP (compPath-filler (merid (pushoutMapₛ terminalCW f (suc n)
                                      (inr x , Iso.fun (IsoSucSphereSusp n) a)))
                             (sym (merid (inl tt))))
         ▷ cong merid (pushoutMapₛ≡funH n x a))
    firstEq (suc n) (push a i) = refl
...
--     CTB' : (n : ℕ) → _
--     CTB' n = BouquetFuns.CTB n
--               (card D n)
--               (α D n)
--               (idEquiv _)

--     BTC' : (n : ℕ) → _
--     BTC' n = BouquetFuns.BTC n
--               (card B n)
--               (α B n)
--               (idEquiv _)

--     p₁ : (n : ℕ) → Path (SplitBouquet (suc n) (card C (suc n)) (card D (suc n)) (card (strictCWskel B') n)) _ _
--     p₁ n = (λ i → inl (push tt (~ i))) ∙ push tt

--     PB : (n : ℕ) → _
--     PB n = Pushout→Bouquet n
--             (card (PushoutCWSkel terminalCW f) n)
--             (α (PushoutCWSkel terminalCW f) n)
--             (e (PushoutCWSkel terminalCW f) n)
...
--               {!!}
--     L1 n x a i =
--       {!SuspBouquet→Bouquet (Fin (card cofibCWskel (suc n)))
-- (λ _ → S₊∙ (suc n))
-- (suspFun
--  (Iso.fun
--   (BouquetIso-gen (suc n) (card cofibCWskel (suc n))
--    (α cofibCWskel (suc n)) (e cofibCWskel (suc n))))
--  (suspFun inr
--   (δ-pre
--    (λ x₁ →
--       invEq
--       (idEquiv (Pushout (α cofibCWskel (suc (suc n))) (λ r → fst r)))
--       (inl x₁))
--    (hcomp
--     (doubleComp-faces
--      (push
--       (α cofibCWskel (suc (suc n))
--        (invEq
--         (finSplit3≃ (card C (suc (suc n))) (card D (suc (suc n)))
--          (pushoutMidCells terminalCW f (suc (suc n))))
--         (inr x)
--         , a)))
--      (λ i →
--         push
...
--     ∂L'≡∂R'-inr n x a =
--       (cong-∙∙ LLL _ _ _ -- cong (cong LLL) {!!} --  _ _ _
--       ∙ (cong₃ _∙∙_∙∙_ refl (λ _ _ → inl tt) refl)
--       ∙ (λ i → cong LLL (push (pushoutMapₛ terminalCW f (suc n)
--                   ((secEq (finSplit3≃ (card C (suc (suc n))) (card D (suc (suc n)))
--                      (pushoutMidCells terminalCW f (suc (suc n)))) (inr x) i)
--                  , (Iso.fun (IsoSucSphereSusp n) a))))
--             ∙∙ (λ _ → inl tt)
--            ∙∙ sym (cong LLL (push (pushoutMapₛ terminalCW f (suc n)
--                   ((secEq (finSplit3≃ (card C (suc (suc n))) (card D (suc (suc n)))
--                      (pushoutMidCells terminalCW f (suc (suc n)))) (inr x) i)
--                  , (Iso.fun (IsoSucSphereSusp n) (ptSn (suc n))))))))
--       ∙ cong₃ _∙∙_∙∙_ refl refl (cong sym (cong (cong LLL ∘ push) (cong (pushoutMapₛ terminalCW f (suc n)) (ΣPathP (refl , IsoSucSphereSusp∙' n)) ∙ refl) ∙ (λ _ _ → inl tt))) -- doubleCompPath≡compPath _ _ _
--       ∙ sym (compPath≡compPath' _ _)
--       ∙ sym (rUnit _)
--       ∙ cong (Bouquet→ΩBouquetSusp (Fin (card D (suc n) +ℕ card B n)) (λ _ → S₊∙ (suc n)))
--              {!Pushout→Bouquet (suc n) (card cofibCWskel (suc n))
--       (α cofibCWskel (suc n))
--       (compEquiv
...
--          (inr x , Iso.fun (IsoSucSphereSusp n) a))))
--          -}!})
--       ∙ (Bouquet→ΩBouquetSuspPresStr _ (f2 n x) (f1 n x) a
--        ∙ cong₂ _∙_ refl
--                (λ _ → Bouquet→ΩBouquetSusp (Fin (card D (suc n) +ℕ card B n))
--                          (λ _ → S₊∙ (suc n)) (Iso.inv (SphereBouquetCellIso n (suc n))
--                           (inl (inr (prefunctoriality.bouquetFunct (suc (suc n))
--                          (cellMap→finCellMap (suc (suc n)) f) flast (inr (x , a)))))))
--        ∙ cong₂ _∙_ (congSphereBouquetCellIsoLemma3 n _)
--                    (congSphereBouquetCellIsoLemma2 n _))
--       ∙ cong₂ _∙_ refl (sym (cong-∙∙ (Iso.inv (SphereBouquetCellIso (suc n) (suc n))) _ _ _
--                         ∙ cong₃ _∙∙_∙∙_ (cong sym congp₁) refl congp₁
--                         ∙ sym (rUnit _)))
--        ∙ sym (cong-∙ (Iso.inv (SphereBouquetCellIso (suc n) (suc n)))
--                        (leftLoop n x a) (rightLoop' n x a))
--        ∙ cong (cong (Iso.inv (SphereBouquetCellIso (suc n) _))) (sym rewr1) -- preboundarypre∂Simpl
--     {-
--         cong (Bouquet→ΩBouquetSusp (Fin (card D (suc n) +ℕ card B n)) (λ _ → S₊∙ (suc n)))
--           (cong (PB (suc n))
--                   (cong (Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f n .Iso.fun)
--                     (cong (PushoutA→PushoutPushoutMap terminalCW f n)
--                       (cong (pushoutMapₛ terminalCW f (suc n))
--                         (ΣPathP (secEq (finSplit3≃ _ _ _) (inr x) , refl)))
--                   ∙ PushoutA→PushoutPushoutMap'≡ n
--                      (pushoutMapₛ terminalCW f (suc n) (inr x , Iso.fun (IsoSucSphereSusp n) a))))
--           ∙ ?) -- ∂L'≡∂R'-inr-main n x a) -- correct order?
--        ∙ Bouquet→ΩBouquetSuspPresStr _ (f2 n x) (f1 n x) a
--        ∙ cong₂ _∙_ refl
--                (λ _ → Bouquet→ΩBouquetSusp (Fin (card D (suc n) +ℕ card B n))
--                          (λ _ → S₊∙ (suc n)) (Iso.inv (SphereBouquetCellIso n (suc n))
...
--        ∙ sym (cong-∙ (Iso.inv (SphereBouquetCellIso (suc n) (suc n)))
--                        (leftLoop n x a) (rightLoop' n x a))
--        ∙ cong (cong (Iso.inv (SphereBouquetCellIso (suc n) _))) (sym rewr1)
-- -}
--       where -- preboundarypre∂Simpl
      
--       nicer : (n : ℕ) → (x : A (strictCWskel B') (suc n)) (a : Susp (S₊ n)) → Pushout (pushoutMapₛ terminalCW f n) fst
--       nicer n x north = inl (inl tt)
--       nicer n x south = PushoutA→PushoutPushoutMapR terminalCW f n (∣ f ∣ (suc (suc n)) (inr x))
--       nicer n x (merid a i) =
...
--            ∙ cong (∣ f ∣ (suc (suc n))) (push (x , a))))) i
--       {-
--         PushoutA→PushoutPushoutMapR terminalCW f n
--           ((({!!} ∙ comm f (suc n) (α B (suc n) (x , a))) ∙ cong (∣ f ∣ (suc (suc n))) (push (x , a))) i) --  (? ∙ ∣ f ∣ (suc (suc n))) (push (x , a))
--           -}
--       {- -- PushoutA→PushoutPushoutMapR terminalCW f n (∣ f ∣ (suc (suc n)) (inr x))
--         hcomp (λ k → {!∣ f ∣ (suc (suc n)) (push (x , a) ?)!})
--               (push (inr {!PushoutA→PushoutPushoutMapR terminalCW f n (∣ f ∣ (suc (suc n)) (inr x))!} , Iso.fun (IsoSphereSusp n) a) i) -- (push ({!!} , (Iso.fun (IsoSphereSusp n) a)) i)
-- -}
--       ch : (n : ℕ) (x : _) (a : _)
--         → PushoutA→PushoutPushoutMap' n (pushoutMapₛ terminalCW f (suc n) (inr x , a))
--         ≡ {!!} -- inl (push {!!} i0) -- inl (α cofibCWskel (suc n) {!x , ?!})
--       ch = {!!}


--       module _ (n : ℕ) where
--         G1 = Pushout→Bouquet (suc n) (card cofibCWskel (suc n))
--                           (α cofibCWskel (suc n))
--                           (compEquiv
--                            (PushoutA→PushoutPushoutMap terminalCW f n ,
--                             isEquivPushoutA→PushoutPushoutMap (strictCWskel B') UnitCWskel
--                             (strictCWskel D') terminalCW f n)
--                            (isoToEquiv (Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f n)))
--         G2 = Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f n .Iso.fun
--         G3 = PushoutA→PushoutPushoutMap' n
--         G4 = pushoutMapₛ terminalCW f (suc n)


--       LLL : {!!}
--       LLL = SuspBouquet→Bouquet (Fin (card cofibCWskel (suc n))) (λ _ → S₊∙ (suc n)) ∘ suspFun (Iso.fun (BouquetIso-gen (suc n) (card cofibCWskel (suc n))
--                 (α cofibCWskel (suc n)) (e cofibCWskel (suc n)))) ∘ suspFun inr ∘ δ-pre inl
--       f1 f2 f3 f4 : (n : ℕ) (x : _)
--         → S₊∙ (suc n) →∙ ⋁gen∙ (Fin (card D (suc n) +ℕ card B n)) (λ _ → S₊∙ (suc n))
--       fst (f1 n x) a = Iso.inv (SphereBouquetCellIso n (suc n))
--         (inl (inr (prefunctoriality.bouquetFunct (suc (suc n))
--                    (cellMap→finCellMap (suc (suc n)) f) flast (inr (x , a)))))
--       snd (f1 zero x) = refl
--       snd (f1 (suc n) x) = refl
--       fst (f2 n x) a = Iso.inv (SphereBouquetCellIso n (suc n)) (inr (preboundarypre∂Simpl B n (inr (x , a))))
--       snd (f2 zero x) = refl
--       snd (f2 (suc n) x) = refl
--       fst (f3 n x) z = G1 n (G2 n (PushoutA→PushoutPushoutMapR terminalCW f n
--                               (α D (suc (suc n)) (x , z) )))
--       snd (f3 n x) = {!preboundarypre∂Simpl B n  ?!}
--       f4 = {!!}

--       M! : (n : ℕ) → Pushout (α (strictCWskel D') (suc n)) fst → SphereBouquet (suc n) (Fin (card cofibCWskel (suc n)))
--       M! n (inl x) = inl tt
--       M! n (inr x) = inl tt
--       M! zero (push a i) = {!!}
--       M! (suc n) (push a i) = {!!}
--       maybe? : (n : ℕ) (a : _) → G1 n (G2 n (PushoutA→PushoutPushoutMapR terminalCW f n a))
--                                  ≡ {!G1 n -- preboundarypre∂Simpl --  cofibCWskel (suc n)!} -- preboundarypre∂Simpl {!cofibCWskel!} ? {!!}  -- Iso.fun sphereBouquetSuspIso (suspFun {!inl-cellMap!} {!G1 n !})
--       maybe? n a = {!G1 n!}
--       {-
--       maybe? n (inl x) = refl
--       maybe? n (inr x) = sym (push (invEq (finSplit3≃ _ _ _) (inr x)))
--       maybe? n (push a i) = {!!}
--         where
--         hha : cong (G1 n ∘ G2 n) ((λ i → inl (inr (α D (suc n) (fst a , Iso.leftInv (IsoSphereSusp n) (snd a) (~ i)))) )
--                                  ∙ push (inl (inr (fst a)) , Iso.fun (IsoSphereSusp n) (snd a)))
--            ≡ {!!}
--         hha = cong-∙ (G1 n ∘ G2 n) _ _ ∙ cong₂ _∙_ (λ _ _ → inl tt) refl
--               ∙ sym (lUnit _)
--               ∙ cong-∙∙ (G1 n) _ _ _
--               ∙ {!G1 n (inl ?)!}  -- sym (lUnit _) ∙ {!!}
-- -}
--       LEM2 : (n : ℕ) (x : _) (a : _)
--         → cong (PushoutA→PushoutPushoutMapR terminalCW f n)
--             (comm f (suc n) (α (strictCWskel B') (suc n) (x , a)) ∙ cong (∣ f ∣ (suc (suc n))) (push (x , a)))
--         ≡ {!comm f (suc n)!}
--       LEM2 n x a = {!!}

--       LEM : (n : ℕ) (x : _) (a : _)
--         → cong (G1 n ∘ G2 n ∘ G3 n ∘ G4 n) (λ i → (inr x , merid a i) ) -- (inr x , Iso.fun (IsoSucSphereSusp n) a))))
--         ≡ {!!}
--       LEM n x a = cong-∙ (G1 n ∘ G2 n ∘ G3 n) _ _
--                 ∙ cong₂ _∙_ (cong-∙ (G1 n ∘ G2 n) _ _
--                             ∙ refl)
--                             refl
--                 ∙ sym (assoc _ _ _)
--                 ∙ cong₂ _∙_ {!!} (cong₂ _∙_ {!!} {!G3 n (inr
--          (∣ f ∣ (suc (suc n))
--           (invEq (e (strictCWskel B') (suc n)) (push (x , a) ?))))!} ∙
--                   {!G3 n (inr
--          (∣ f ∣ (suc (suc n))
--           (invEq (e (strictCWskel B') (suc n)) (push (x , a) ?))))!})
--                 ∙ {!!}

--       congp₁ : cong (Iso.inv (SphereBouquetCellIso (suc n) (suc n))) (p₁ n) ≡ refl
--       congp₁ = cong-∙ (Iso.inv (SphereBouquetCellIso (suc n) (suc n))) _ _ ∙ sym (rUnit refl)
--       module _ (n : ℕ) (x : _) (a : _) where
--         rightLoop : Path (SplitBouquet (suc n) (card C (suc n)) (card D (suc n))
--                   (card (strictCWskel B') n)) (inl (inr (inl tt))) (inl (inr (inl tt)))
--         rightLoop i =
--           inl (inr (Bouquet→ΩBouquetSusp (Fin _) (λ _ → S₊∙ (suc n))
--             (prefunctoriality.bouquetFunct (suc (suc n))
--               (cellMap→finCellMap (suc (suc n)) f) flast (inr (x , a))) i))
   
--         leftLoop rightLoop' :
--           Path (SplitBouquet (suc n) (card C (suc n)) (card D (suc n))
--                   (card (strictCWskel B') n)) (inr (inl tt)) (inr (inl tt))
--         leftLoop i = inr (Bouquet→ΩBouquetSusp (Fin _) (λ _ → S₊∙ (suc n))
--                              (preboundarypre∂Simpl B n (inr (x , a))) i)
--         rightLoop' = sym (p₁ n) ∙∙ rightLoop ∙∙ p₁ n

--       rewr1 : cong (∂RR n) (λ i → inr (inr (x , (merid a i))))
--             ≡ leftLoop n x a
--             ∙ rightLoop' n x a
--       rewr1 = (cong₂ _∙_ ((cong₃ _∙∙_∙∙_
--                      (cong sym ((assoc _ _ _ ∙ sym (rUnit _)) ∙ sym (rUnit refl)))
--                      ((cong-∙ (λ w → inr (bouquetSusp→ (preboundarypre∂Simpl B n) (inr (x , w))))
--                              (merid a)
--                              (sym (merid (ptSn (suc n))))
--                       ∙ cong₂ _∙_ refl (λ j i → inr (Bouquet→ΩBouquetSusp (Fin _) (λ _ → S₊∙ (suc n))
--                                   (preboundarypre∂Simpl (strictCWskel B') n (push x (~ j))) (~ i)))
--                       ∙ cong₂ _∙_ refl (λ j i → inr (Bouquet→ΩBouquetSusp (Fin _) (λ _ → S₊∙ (suc n))
--                                               (sphereBouquetSuspIso∙ j ) (~ i)))
--                       ∙ sym (rUnit _)))
--                      ((assoc _ _ _ ∙ sym (rUnit _)) ∙ sym (rUnit refl)))
--                    ∙ sym (rUnit _))
--         ((cong₃ _∙∙_∙∙_
--          (cong sym (sym (lUnit _) ∙ sym (lUnit _) ∙ λ _ → p₁ n))
--               (cong-∙ (λ w → inl (inr (bouquetSusp→  (prefunctoriality.bouquetFunct (suc (suc n))
--                      (cellMap→finCellMap (suc (suc n)) f) flast) (inr (x , w)))))
--                      (merid a) (sym (merid (ptSn (suc n))))
--               ∙ cong₂ _∙_ refl
--                  ((λ j i → inl (inr (Bouquet→ΩBouquetSusp (Fin (card (strictCWskel D') (suc n)))
--                                        (λ _ → S₊∙ (suc n))
--                                        (CTB' (suc n)
--                                          (prefunctoriality.fn+1/fn (suc (suc n))
--                                            (cellMap→finCellMap (suc (suc n)) f) flast
--                                             (BTC' (suc n) (push x (~ j))))) (~ i)))))
--               ∙ sym (rUnit _))
--               (sym (lUnit _) ∙ sym (lUnit _) ∙ λ _ → p₁ n)))
--         ∙ refl
--         ∙ refl)
--       {- cong₂ _∙_
--                    ((cong₃ _∙∙_∙∙_
--                      (cong sym ((assoc _ _ _ ∙ sym (rUnit _)) ∙ sym (rUnit refl)))
--                      (cong-∙ (λ w → inr (bouquetSusp→ (pre∂Improved B n) (inr (x , w))))
--                              (merid a)
--                              (sym (merid (ptSn (suc n))))
--                       ∙ cong₂ _∙_ refl
--                         (λ j i → inr (Bouquet→ΩBouquetSusp (Fin _) (λ _ → S₊∙ (suc n))
--                                   (pre∂Improved (strictCWskel B') n (push x (~ j))) (~ i)))
--                       ∙ sym (rUnit _))
--                      ((assoc _ _ _ ∙ sym (rUnit _)) ∙ sym (rUnit refl)))
--                    ∙ sym (rUnit _))
--                    (cong₃ _∙∙_∙∙_
...
-- --                         ∘ pre∂Improved cofibCWskel (suc n)
-- --                         ∘ Iso.inv (SphereBouquetCellIso (suc n) _)
-- --     ∂R n = ((((λ x → inl (inr x))
-- --                           , (λ i → inl (push tt (~ i))) ∙ push tt)
-- --                            ∘∙ (pre∂Improved D (suc n) , refl))
-- --                            ∘∙ foldR∙)
-- --                        ∨→ SphereBouquet∙Π
-- --                             (((inr , refl) ∘∙ (bouquetSusp→ (pre∂Improved B n) , refl)))
-- --                             (((λ x → inl (inr x)) , (λ i → inl (push tt (~ i))) ∙ push tt)
-- --                               ∘∙ (bouquetSusp→ (prefunctoriality.bouquetFunct _ (cellMap→finCellMap (suc (suc n)) f) flast) , refl))
-- -- -- preboundarypre∂Simpl
-- --     GH : (n : ℕ) (x : _) → preboundary.pre∂ cofibCWskel (suc n) ((BouquetFuns.CTB (suc (suc n))
-- --                                                 (preboundary.An+1 cofibCWskel (suc n))
-- --                                                   ((fst (e cofibCWskel (suc n))) ∘ α cofibCWskel (suc (suc n)))
-- --                                              (idEquiv _)) x)
-- --                           ≡ SuspBouquet→Bouquet (Fin (preboundary.An cofibCWskel (suc n))) (λ _ → S₊∙ (suc n))
-- --                                (suspFun (BouquetFuns.CTB (suc n) (preboundary.An cofibCWskel (suc n))
-- --                                            (preboundary.αn cofibCWskel (suc n))
-- --                                            (idEquiv _))
-- --                                         (suspFun inr
-- --                                           (δ-pre inl x)))
-- --     GH n (inl x) = {!!}
...
-- --                        (card D (suc (suc n))) (card (strictCWskel B') (suc n))
-- --          → SphereBouquet (suc (suc n)) (Fin (card cofibCWskel (suc n)))
-- --     ∂L' n =  pre∂Improved cofibCWskel (suc n)
-- --                         ∘ Iso.inv (SphereBouquetCellIso (suc n) _)
...
-- --             , (Iso.fun (IsoSphereSusp (suc n)) a))))))!} -- Bouquet→ΩBouquetSusp (Fin (card D (suc n) + card B n)) (λ _ → S₊∙ (suc n))
-- --         --       ((pre∂Improved cofibCWskel (suc n)
...
-- --       snd (f1 zero x) = refl
-- --       snd (f1 (suc n) x) = refl
-- --       fst (f2 n x) a = Iso.inv (SphereBouquetCellIso n (suc n)) (inr (pre∂Improved B n (inr (x , a))))
-- --       snd (f2 zero x) = refl
-- --       snd (f2 (suc n) x) = refl
...
-- --                   (card (strictCWskel B') n)) (inr (inl tt)) (inr (inl tt))
-- --         leftLoop i = inr (Bouquet→ΩBouquetSusp (Fin _) (λ _ → S₊∙ (suc n))
-- --                              (pre∂Improved B n (inr (x , a))) i)
-- --         rightLoop' = sym (p₁ n) ∙∙ rightLoop ∙∙ p₁ n

-- --       rewr1 : cong (∂R n) (λ i → inr (inr (x , (merid a i))))
-- --             ≡ leftLoop n x a
-- --             ∙ rightLoop' n x a
-- --       rewr1 = cong₂ _∙_
-- --                    ((cong₃ _∙∙_∙∙_
...
-- --       F0 : (x : _) → Susp (S₊ 0) → _
-- --       F0 x w =  PB (suc zero)
-- --             (Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f zero .Iso.fun
-- --              (PushoutA→PushoutPushoutMap' zero
-- --               (pushoutMapₛ terminalCW f (suc zero)
-- --                (inr x , w))))
...
-- --       ∂L'≡∂R'-inr-main₁-l : (x : _)
-- --         → cong (F0 x) (merid false ∙ sym (merid true))
-- --           ≡ cong (∙Π  (f1 zero x)  (f2 zero x).fst) loop
-- --       ∂L'≡∂R'-inr-main₁-l x =
-- --         (cong-∙ (F0 x) (merid false) (sym (merid true))
-- --         ∙ cong₂ _∙_ (cong-∙ PBF _ _) (cong sym (cong-∙ PBF _ _)
-- --                        ∙ symDistr _ _)
-- --         ∙ assoc _ _ _
-- --         ∙ cong₂ _∙_ (sym (assoc _ _ _)
-- --                   ∙ cong₂ _∙_ refl (sym (cong-∙ (λ x → PBF (inr (∣ f ∣ (suc (suc zero)) x))) _ _))
-- --                   ∙ refl) refl
-- --         ∙ cong₂ _∙_ refl (cong sym ((cong-∙ (PB (suc zero)
-- --                                            ∘ Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f zero
-- --                                            .Iso.fun) _ _ ∙ refl))
-- --                                  ∙ symDistr _ _)
-- --         ∙ assoc _ _ _)
-- --         {-
-- --         ∙  cong₂ _∙_ ((cong-∙ PBF _ _ ∙ cong₂ _∙_ refl
-- --                        (rUnit _
-- --                       ∙ cong₂ _∙_ refl
-- --                           (sym (rCancel (cong (λ x → PBF (inr (∣ f ∣ (suc (suc zero)) x)))
-- --                                 (sym (push (x , true))))))
-- --                       ∙ assoc _ _ _
-- --                       ∙ cong₂ _∙_ (sym (cong-∙ (λ x → PBF (inr (∣ f ∣ (suc (suc zero)) x)))
-- --                                     (push (x , false)) (sym (push (x , true))))) refl))
-- --                     ∙ assoc _ _ _)
-- --                      ((cong sym (cong-∙ PBF _ _
-- --                        ∙ cong₂ _∙_ refl
-- --                          (rUnit _
-- --                          ∙ (cong₂ _∙_ refl
-- --                            (sym (rCancel (cong (λ x → PBF (inr (∣ f ∣ (suc (suc zero)) x)))
...
-- --                          ∙ cong₂ _∙_ (sym (cong-∙ (λ x → PBF (inr (∣ f ∣ (suc (suc zero)) x)))
-- --                                     (push (x , true)) (sym (push (x , false))))) refl))
-- --                         ∙ assoc _ _ _))
-- --                     ∙ (symDistr _ _)
-- --                     ∙ refl)
-- --         ∙ sym (assoc _ _ _) -- sym (assoc _ _ _) -- (λ i → {!!} ) -- cong₂ _∙_ {!!} {!!}
-- --         ∙ (cong₂ _∙_ refl (assoc _ _ _)
-- --         ∙ cong₂ _∙_ {!!} (cong₂ _∙_ refl (symDistr _ _)
-- --                          ∙ assoc _ _ _
-- --                          ∙ cong₂ _∙_ (cong₂ _∙_ (sym (cong-∙ (λ x → PBF (inr (∣ f ∣ 2 x))) _ _)) refl ∙ rCancel _) refl
-- --                          ∙ sym (lUnit _))
-- --         ∙ {!!}) -- cong₂ _∙_ {!!} {!!} -- cong₂ _∙_ {!!} {!!}
-- --         -}
-- --         ∙ cong₂ _∙_ (cong₂ _∙_ ((cong₂ _∙_
-- --           (cong-∙ (PB (suc zero) ∘ Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f zero .Iso.fun) _ _) refl)
...
-- --        (λ x₁ →
-- --           PB 1
-- --           (Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f zero .Iso.fun x₁))
-- --        (λ i →
-- --           PushoutA→PushoutPushoutMapR terminalCW f zero
-- --           (comm f 1 (α (strictCWskel B') 1 (x , false)) (i ∧ ~ j)))
-- --        ∙
-- --        cong (λ x₁ → PBF (inr (∣ f ∣ 2 x₁)))
-- --        ((λ i → invEq (e (strictCWskel B') 1) (push (x , false) (i ∨ ~ j))) ∙
-- --         (λ i → invEq (e (strictCWskel B') 1) (push (x , true) (~ i ∨ ~ j)))))
-- --       ∙
-- --       sym
-- --       (cong
-- --        (λ x₁ →
...
-- --        (λ i →
-- --           PushoutA→PushoutPushoutMapR terminalCW f zero
-- --           (comm f 1 (α (strictCWskel B') 1 (x , true)) (i ∧ ~ j)))))!}
-- --         {- cong₂ _∙_
-- --           (((((sym (assoc _ _ _) ∙ sym (doubleCompPath≡compPath _ _ _))
-- --             ∙ {!!})
-- --           ∙ {!!}
-- --           ∙ cong₃ _∙∙_∙∙_
-- --             ((sym (cong-∙ (λ x → Iso.inv (sphereBouquetIso zero)
-- --                     (SplitBouquet→sumSphereBouquet zero (inl (inr (CTB' (suc zero) x))))) _ _)))
-- --             refl
-- --             (refl
-- --             ∙ cong sym (sym (cong-∙ (λ x → Iso.inv (sphereBouquetIso zero) (SplitBouquet→sumSphereBouquet zero (inl (inr (CTB' (suc zero) x))))) _ _))))
-- --           ∙ sym (cong-∙∙ (λ x → Iso.inv (sphereBouquetIso zero)
-- --                     (SplitBouquet→sumSphereBouquet zero
-- --                       (inl (inr (CTB' (suc zero) (prefunctoriality.fn+1/fn 2 (cellMap→finCellMap 2 f) flast x)))))) _ _ _) ) ∙ rUnit _) -- sym (cong-∙ (λ x → Iso.inv (SphereBouquetCellIso zero (suc zero)) (inr x)) _ _)  ∙ rUnit _)
-- --           ({!!} ∙ ((λ j i → fst (f2 zero x) (loop i))) ∙ rUnit _)
-- --           -}
-- --           where
-- --           L12 : {!cong (PB (suc zero)) (cong (Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f zero .Iso.fun) ?)!}
-- --           L12 = {!!}
-- --           lem1 : cong (fst (f1 zero x)) loop ≡ {!!}
-- --           lem1 = cong-∙∙ (λ x → Iso.inv (sphereBouquetIso zero) (SplitBouquet→sumSphereBouquet zero
-- --                                     (inl (inr (CTB' (suc zero)
-- --                                      (prefunctoriality.fn+1/fn (suc (suc zero))
-- --                                        (cellMap→finCellMap (suc (suc zero)) f) flast x)))))) _ _ _
-- --                ∙∙ cong₃ _∙∙_∙∙_
-- --                         (cong-∙ (λ x → Iso.inv (sphereBouquetIso zero) (SplitBouquet→sumSphereBouquet zero
-- --                                     (inl (inr (CTB' (suc zero) x))))) _ _
-- --                         ∙ (cong₂ _∙_ (λ _ _ → inl tt) refl))
-- --                         refl
-- --                         (cong sym (cong-∙ (λ x → Iso.inv (sphereBouquetIso zero) (SplitBouquet→sumSphereBouquet zero
-- --                                     (inl (inr (CTB' (suc zero) x))))) _ _))
-- --                ∙∙ ({!!}
-- --                  ∙ {!!})
-- --           PBF = PB (suc zero)
...
-- --           PBF-push : (x : _) → cong PBF (push x) ≡ {!Iso.inv (sphereBouquetIso zero) (SplitBouquet→sumSphereBouquet zero
-- --  (inl
-- --   (inr
-- --    (Pushout→Bouquet 1 (card D 1) (α D 1)
-- --     (idEquiv (Pushout (α D 1) (λ r → fst r)))
-- --     (∣ f ∣ (fst (fsuc (1 , tt)))
-- --      (invEq
-- --       (e (strictCWskel B')
-- --        (prefunctoriality.n 2 (cellMap→finCellMap 2 f) (1 , tt)))
-- --       ((push (x , false) ∙ (λ i₁ → push (x , true) (~ i₁)))!}
-- --           PBF-push = {!PBF (inr ?)!}

-- --           compute : cong PBF (push (α (strictCWskel B') 1 (x , false)))
-- --                  ∙ cong (λ x₂ → PBF (inr (∣ f ∣ (suc (suc zero)) x₂))) (push (x , false))
-- --                ≡ cong (PB (suc zero) ∘ Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f zero .Iso.fun)
-- --                       {!PushoutA→PushoutPushoutMapLR terminalCW f zero (α (strictCWskel B') 1 (x , false)) !} -- cong (λ x → PBF (inr (∣ f ∣ (suc (suc zero)) x)))  (push (x , false)) ≡ {!!}
-- --           compute = {!PushoutA→PushoutPushoutMap' zero (push (α (strictCWskel B') 1 (x , false)) ?)!}
-- --                   ∙ {!PBF (push (α (strictCWskel B') 1 (x , false)) ?)!}
-- --         {-
-- --         ∙ cong₂ _∙_
-- --            (((refl
-- --            ∙ cong₂ _∙_ refl refl)
-- --            ∙ sym (cong-∙ (Iso.inv (SphereBouquetCellIso zero 1)) _ _))
-- --            ∙ rUnit _)
-- --            ((({!!}
-- --            ∙ cong₃ _∙∙_∙∙_
-- --                ({!!} ∙ sym (cong-∙ (λ w → Iso.inv (SphereBouquetCellIso zero (suc zero))
-- --                                               (inl (inr (CTB' (suc zero) w))))
-- --                                    (push (∣ f ∣ (suc zero) (α B (suc zero) (x , false))))
-- --                                    λ i → inr (comm f (suc zero) (α B (suc zero) (x , false)) i)))
               
-- --                {!!}
-- --               (cong sym (sym (cong-∙ (λ w → Iso.inv (SphereBouquetCellIso zero (suc zero))
-- --                                               (inl (inr (CTB' (suc zero) w))))
-- --                                    (push (∣ f ∣ (suc zero) (α B (suc zero) (x , true))))
-- --                                    λ i → inr (comm f (suc zero) (α B (suc zero) (x , true)) i)))))
-- --            ∙ sym (cong-∙∙ (λ x → Iso.inv (SphereBouquetCellIso zero (suc zero))
-- --               (inl (inr (CTB' (suc zero)
-- --               (prefunctoriality.fn+1/fn 2 (cellMap→finCellMap 2 f) flast x))))) _ _ _)) -- sym (cong-∙ (λ x → Iso.inv (SphereBouquetCellIso zero (suc zero)) (inl (inr x))) _ _))
-- --            ∙ rUnit _)
-- --            -}

-- --       ∂L'≡∂R'-inr-main : (n : ℕ) (x : _) (a : _)
-- --         → PB (suc n)
-- --             (Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f n .Iso.fun
-- --              (PushoutA→PushoutPushoutMap' n
-- --               (pushoutMapₛ terminalCW f (suc n)
-- --                (inr x , Iso.fun (IsoSucSphereSusp n) a))))
-- --         ≡ ∙Π (f2 n x) (f1 n x) .fst a
-- --       ∂L'≡∂R'-inr-main zero x base = refl
-- --       ∂L'≡∂R'-inr-main zero x (loop i) j = {!!} -- ∂L'≡∂R'-inr-main₁-l x j i
-- --       ∂L'≡∂R'-inr-main (suc n) x north = refl
-- --       ∂L'≡∂R'-inr-main (suc n) x south j = {!!}
-- --       {-
-- --         PB (suc (suc n))
-- --       (Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f (suc n) .Iso.fun
-- --        (PushoutA→PushoutPushoutMapR terminalCW f (suc n) {!comm f (suc (suc n)) ? (~ j)!}))
-- --        -}
-- --       ∂L'≡∂R'-inr-main (suc n) x (merid a i) = {!!}
-- --         where
-- --         TET : (a : _) → cong (Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f (suc n) .Iso.fun)
-- --                           (PushoutA→PushoutPushoutMapLR terminalCW f (suc n) a)
-- --                        ≡ {!!} ∙ {!PB (suc (suc n)) ∘ (Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f (suc n) .Iso.fun)!}
-- --         TET = {!!}
-- --         TE : (a : _)
-- --           → cong (PB (suc (suc n))
-- --                   ∘ Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f (suc n) .Iso.fun)
...
-- --           ≡ refl
-- --         TE (inl x) = refl
-- --         TE (inr x) = {!cong (Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f (suc n) .Iso.fun)
-- --               (PushoutA→PushoutPushoutMapLR terminalCW f (suc n) ?)!}
-- --         TE (push a i) = {!!}
-- -- {-
-- --         h2 : cong (λ w → PB (suc (suc n))
-- --             (Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f (suc n) .Iso.fun
-- --              (PushoutA→PushoutPushoutMap' (suc n)
-- --               (pushoutMapₛ terminalCW f (suc (suc n))
-- --                (inr x , w))))) (merid a)
-- --             ≡ {!!}
-- --         h2 = cong-∙ (PB (suc (suc n)) ∘ Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f (suc n) .Iso.fun ∘ PushoutA→PushoutPushoutMap' (suc n)) _ _
-- --            ∙ cong₂ _∙_ (cong-∙ (PB (suc (suc n)) ∘ Iso-Pushoutα-PushoutPushoutMapₛ terminalCW f (suc n) .Iso.fun) _ _
-- --                        ∙ cong₂ _∙_ {!!} {!!})
-- --                        {!!} -- sym (rUnit _)
-- --            ∙ {!!}
-- --         help : cong (∙Π (f2 (suc n) x) (-Π (f1 (suc n) x)) .fst) (merid a) ≡ _
-- --         help = cong₂ _∙_ (sym (rUnit _)
-- --                        ∙ cong-∙ (fst (f2 (suc n) x)) (merid a) (sym (merid (ptSn _)))
-- --                        ∙ refl)
-- --                        (sym (rUnit _)
-- --                        ∙ cong-∙ (-Π (f1 (suc n) x) .fst) (merid a) (sym (merid (ptSn _)))
-- --                        ∙ cong₂ _∙_ refl (cong (cong (f1 (suc n) x .fst)) (rCancel (merid (ptSn (suc n)))))
-- --                        ∙ sym (rUnit _)
-- --                        ∙ cong sym (cong-∙ (fst (f1 (suc n) x)) (merid a) (sym (merid (ptSn _)))) ∙ symDistr _ _)
-- --              ∙ (sym (assoc _ _ _) ∙ cong₂ _∙_ refl (assoc _ _ _ ∙ cong₂ _∙_ {!!} refl ∙ {!!}))
-- --              ∙ {!!}
-- -- -}
...
-- --     ∂L'≡∂R'-inr (suc n) x a =
-- --         {!!}
-- --       ∙ {!cong cong (∂L' zero) (λ i → inr (inr (x , merid a i)))!} -- cong₂ _∙_ {!!} {!!}
-- --       where
-- --       p₁ : Path (SplitBouquet (suc (suc n)) (card C (suc (suc n))) (card D (suc (suc n))) (card (strictCWskel B') (suc n))) _ _
-- --       p₁ =  (λ i → inl (push tt (~ i))) ∙ push tt

-- --       rewr1 : cong (∂R (suc n)) (λ i → inr (inr (x , (merid a i))))
-- --             ≡ {!!}
-- --       rewr1 = cong₂ _∙_
-- --                     (cong₃ _∙∙_∙∙_
-- --                       (cong sym ((assoc _ _ _ ∙ sym (rUnit _)) ∙ sym (rUnit refl)))
-- --                        (cong-∙ (λ w → inr (bouquetSusp→ (pre∂Improved B (suc n)) (inr (x , w))))
-- --                          (merid a) (sym (merid north))
-- --                      ∙ (sym (rUnit _)))
-- --                       ((assoc _ _ _ ∙ sym (rUnit _)) ∙ sym (rUnit refl))
-- --                     ∙ sym (rUnit _))
-- --                     (cong₃ _∙∙_∙∙_
-- --                       (cong sym (sym (lUnit _) ∙ sym (lUnit _) ∙ λ _ → p₁))
-- --                            (cong-∙ (λ w → inl (inr (bouquetSusp→  (prefunctoriality.bouquetFunct (suc (suc (suc n)))
-- --                                   (cellMap→finCellMap (suc (suc (suc n))) f) flast) (inr (x , w)))))
-- --                                   (merid a) (sym (merid north))
-- --                            ∙ sym (rUnit _))
-- --                            (sym (lUnit _) ∙ sym (lUnit _) ∙ λ _ → p₁)
-- --                     ∙ refl)
-- --             ∙ {!bouquetSusp→ (pre∂Improved B (suc n)) (inr (x , (merid a ?)))!}
-- -- -}    
-- -- --     {-
...
-- -- --       {- cong₂ _∙_ (cong₃ _∙∙_∙∙_ (cong sym (sym (lUnit _) ∙ sym (lUnit p₁) ∙ refl))
-- -- --                      (cong-∙ (λ w → inl (inr
-- -- --                                       (bouquetSusp→
-- -- --                                        (prefunctoriality.bouquetFunct (suc (suc n))
-- -- --                                         (cellMap→finCellMap (suc (suc n)) f) flast)
