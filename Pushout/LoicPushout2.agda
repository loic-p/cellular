{-# OPTIONS --cubical #-}
module Pushout.LoicPushout2 where
...
module PASH (ℓ : Level) (B C D : CWskel ℓ)
  ...
  strictEquiv : (n : ℕ) (C : CWskel ℓ) → strict²A C n ≃ strictA C n
  strictEquiv zero C = idEquiv _
  strictEquiv (suc zero) C = idEquiv _
  strictEquiv (suc (suc n)) C = pushoutEquiv _ _ _ _
    (isoToEquiv (prodIso idIso (invIso (IsoSphereSusp n))))
    (invEquiv (e C n))
    (idEquiv _)
    (funExt (λ x → retEq (e C n) (α C (suc n) (fst x , Iso.inv (IsoSphereSusp n) (snd x)))))
    refl
  ...
  -- strictMapStrict : {X Y : CWskel ℓ} (f : cellMap X Y)
  --   → (n : ℕ) (x : _)
  --     → fst (invEquiv (e Y n)) (strictMap {X = X} {Y} f (suc n) x)
  --        ≡ ∣ f ∣ (suc n) (fst (invEquiv (e X n)) x)
  -- strictMapStrict f n (inl x) = comm f n x
  -- strictMapStrict {X = X} {Y = Y} f n (inr x) =
  --   retEq (e Y n) (∣ f ∣ (suc n) (invEq (e X n) (inr x)))
  -- strictMapStrict {X = X} {Y} f n (push a i) j =
  --   hcomp (λ k → λ {(i = i0) → {!!}
  --                  ; (i = i1) → retEq (e Y n)  (∣ f ∣ (suc n) (invEq (e X n) (push a k))) j
  --                  ; (j = i0) → invEq (e Y n) (doubleCompPath-filler
  --                                  ((λ i → secEq (e Y n) (inl (∣ f ∣ n (α X n a))) (~ i)))
  --                                  (λ i → e Y n .fst (f .comm n (α X n a) i))
  --                                  (λ i → e Y n .fst (∣ f ∣ (suc n) (invEq (e X n) (push a i)))) k i)
  --                  ; (j = i1) → ∣ f ∣ (suc n) (invEq (e X n) (push a (k ∧ i)))})
  --         (retEq (e Y n) {!f .comm n (α X n a) i!} j) -- compPath-filler' (comm f n (α (strictCWskel X) n a)) (cong (∣ f ∣ (suc n)) (push a)) (~ j) i
  ...
  pushoutMapₛ** : (n : ℕ) → (((A C (suc n)) ⊎ (A B n)) ⊎ (A D (suc n))) × S₊ n → pushoutA' (suc n)
  pushoutMapₛ** n (a , x) = pushoutMapₛ' n (a , Iso.fun (IsoSphereSusp n) x)

  pushoutMapₛ* : (n : ℕ) → (Fin (pushoutCells' n)) × (S⁻ n) → pushoutA' n
  pushoutMapₛ* zero = pushoutMap₀'
  pushoutMapₛ* (suc n) (a , x) = pushoutMapₛ' n (finSplit3 (card C (suc n)) (card B n) (card D (suc n)) a
  ...
  pushoutMapₛ**Iso : (n : ℕ) → Iso (Pushout (pushoutMapₛ** n) fst) (Pushout (pushoutMapₛ' n) fst)
  pushoutMapₛ**Iso n = pushoutIso _ _ _ _ (isoToEquiv (prodIso idIso (IsoSphereSusp n))) (idEquiv _) (idEquiv _) refl refl

  -- pushoutMapₛ**Iso↑ : (n : ℕ) → strictPushout n → Pushout (pushoutMapₛ** (suc n)) fst
  -- pushoutMapₛ**Iso↑ n x = inl {!!}

  -- pushoutIsoₛ' : (n : ℕ) → Iso (strictPushout n) (Pushout (pushoutMapₛ** n) fst)
  -- pushoutIsoₛ' = {!!}


  backEq : (n : ℕ) → ((A C (suc n) ⊎ A B n) ⊎ A D (suc n)) × Susp (S⁻ n) ≃
      Fin (card C (suc n) +ℕ card B n +ℕ card D (suc n)) × S₊ n
  backEq n = isoToEquiv (prodIso (invIso (equivToIso (finSplit3≃ (card C (suc n)) (card B n) (card D (suc n))))) (invIso ( IsoSphereSusp n)))

  mapComm : (n : ℕ) (x : _) → pushoutMapₛ' n (invEq (backEq n) x) ≡ pushoutMapₛ* (suc n) x
  mapComm n x =  refl

  pushoutMapIso2 : (n : ℕ) → Iso (Pushout (pushoutMapₛ' n) fst) (Pushout (pushoutMapₛ* (suc n)) fst)
  pushoutMapIso2 n =
    invIso (pushoutIso _ _ _ _ (invEquiv (backEq n)) (idEquiv _) (finSplit3≃ (card C (suc n)) (card B n) (card D (suc n))) refl refl)

  e1 : (n : ℕ) → strictPushout n ≃ Pushout (pushoutMapₛ* (suc n)) fst
  e1 n = compEquiv (isoToEquiv (pushoutIsoₛ n)) (isoToEquiv (pushoutMapIso2 n))

  e↑ : (n : ℕ) → Pushout (pushoutMapₛ* (suc n)) fst → Pushout (pushoutMapₛ* (suc (suc n))) fst
  e↑ n x = inl {!invEq (e1 n) x !}

  -- cofibStrict : (n : ℕ) → {!pushoutMapₛ** n!} -- BouquetFuns.CTB -- BouquetFuns
  -- cofibStrict = {!!}

  -- pushoutStrictEquiv : (n : ℕ) → (strictPushout n) ≃ (pushoutA' (suc (suc n)))
  -- pushoutStrictEquiv n =
  --   pushoutEquiv _ _ _ _ (invEquiv (e B n)) (strictEquiv (2+ n) C) (strictEquiv (2+ n) D)
  --     (λ x i → {!!})
  --     {!pushoutIsoₛ!}
  ...
  -- pushoutSkel = pushoutA' , (pushoutCells' , pushoutMapₛ* , (B .snd .snd .snd .fst)
  --   , λ { zero → {!!}
  --   ; (suc n) → compEquiv (compEquiv (invEquiv (pushoutStrictEquiv n)) (isoToEquiv (pushoutIsoₛ n))) (isoToEquiv (pushoutMapIso2 n))})
