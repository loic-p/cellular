...
module Pushout.Rebuttal.Pushout where
...
module _ (ℓ : Level) (B C D : CWskel ℓ)
  ...
  pushoutCWSkel : CWskel ℓ
  fst pushoutCWSkel = pushoutA
  fst (snd pushoutCWSkel) = pushoutCells
  fst (snd (snd pushoutCWSkel)) = pushoutMap
  fst (snd (snd (snd pushoutCWSkel))) = B .snd .snd .snd .fst
  snd (snd (snd (snd pushoutCWSkel))) n =
    compEquiv {!!} (compEquiv (isoToEquiv (pushoutIsoₛ n)) {!!})
